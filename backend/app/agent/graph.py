...
async def stream_agent_response(
    ...
    Uses graph.astream() to get state updates after each node runs.
    Each update may contain new AI messages — we extract the text
    and yield it. This works with the multi-agent architecture where
    responses can come from the supervisor (direct messages) or
    sub-agents (LLM-generated), regardless of how they were produced.

    NOTE: This yields complete messages per node, not token-by-token.
    True token-level streaming in a multi-agent graph requires
    astream_events with carefully scoped event names. For the voice
    pipeline, message-level granularity is sufficient — the TTS
    sentence buffer smooths out delivery.
    ...
    Yields:
        Text chunks as they're produced by whichever agent responds.
    """
    graph = await _get_or_build_graph()
    config = {"configurable": {"thread_id": thread_id}}
    input_message = {"messages": [("human", message)]}

    # Track messages we've already yielded so we don't repeat them.
    # Each astream update includes the FULL messages list (not just new ones),
    # so we need to track what's new.
    seen_count = 0
